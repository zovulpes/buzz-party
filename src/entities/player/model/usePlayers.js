...
export const usePlayers = () => {
    ...
    try {
      ...
      if (saved) {
        ...
        return DEFAULT_PLAYERS.map((def) => {
          const savedPlayer = Array.isArray(parsed)
            ? parsed.find((p) => p.id === def.id)
            : null;
  ...
  const [loading] = useState(false); // localStorage синхронен → сразу готово
  ...
  useEffect(() => {
    try {
      ...
      // Обновляем error асинхронно, чтобы избежать синхронного setState внутри эффекта
      setTimeout(() => setError("Storage quota exceeded or disabled"), 0);
    }
  }, [players]);
